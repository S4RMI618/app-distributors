<x-app-layout>
                ...
                <div class="p-4 py-5 sm:px-6">
                    <form action="{{ route('orders.update', $order->id) }}" method="POST">
                        ...
                        <div class="mt-4">
                            ...
                            <div id="selected_products_list" class="block mt-1 w-full rounded-md">
                                ...
                                <div class="product-item flex flex-wrap justify-between items-center mt-2 p-2 bg-gray-100 rounded-md">
                                    <span class="product-name font-medium w-full text-center text-1xl bg-gray-200 rounded-md p-1">{{ $product->name }} - {{ $product->code }}</span>
                                    <div class="flex w-full items-center justify-between md:justify-around gap-4 py-2">
                                        <div class="flex items-center">
                                            ...
                                            <input type="number" name="quantities[]"
                                                class="quantity-input block w-14 md:w-16 rounded-md border border-gray-300 px-2 py-1 tex-center"
                                                min="1" value="{{ $product->pivot->quantity }}" onchange="updateTotals()">
                                        </div>
                                        <div class="product-subtotal font-medium">
                                            Subtotal: $<span class="subtotal-amount">0.00</span>
                                        </div>
                                        <input type="hidden" name="products[]" class="product-input" value="{{ $product->id }}">
                                        <input type="hidden" name="product_prices[]" class="product-price-input" value="{{ $product->base_price }}">
                                        <input type="hidden" class="product-tax-rate" value="{{ $product->tax_rate }}">
                                        <input type="hidden" class="product-base-price" value="{{ $product->base_price }}">
                        ...
                        <div class="mt-4">
                            ...
                            <textarea id="observations" name="observations" class="block mt-1 w-full rounded-md p-1 px-2 md:p-4" rows="3" placeholder="Escribe aquí las observaciones...">{{ $order->observations }}</textarea>
    ...
    <script>
        ...
        updateTotals(); // Actualizar totales al cargar la página
